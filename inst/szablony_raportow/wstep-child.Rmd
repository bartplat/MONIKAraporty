...
cat("\n\n## Wykorzystanie raportu\n\n",
    ...
    "*Raport roczny z monitoringu karier absolwentów szkoły ", szk$dane_szkoly[[1]]$nazwa, ". Monitoring na podstawie danych rejestrowych. Warszawa: Instytut Badań Edukacyjnych - Państwowy Instytut Badawczy, ",
    params$rok + 1,
    ".*\n\n",
...
cat("\n\n## Źródła i przetwarzanie danych\n\n",
    ...
    "Dane te przekazywane są do Centrum Informatycznego Edukacji, gdzie są przetwarzane i anonimizowane, a następnie do Instytutu Badań Edukacyjnych - Państwowego Instytutu Badawczego (IBE-PIB). Eksperci Instytutu opracowują wyniki monitoringu karier absolwentów, w szczególności w formie raportu krajowego, raportów wojewódzkich i branżowych oraz raportów dla szkół, takich jak ten, który mają Państwo przed sobą. Raporty dla poszczególnych szkół są w IBE-PIB tworzone automatycznie, na podstawie opracowanego przez ekspertów wzoru.",
    sep = "")
```
...
cat("\n\n## Absolwenci Państwa szkoły uwzględnieni w raporcie\n\n",
    ...
    params$rok + 1,
    " roku. Absolwentami są uczniowie ostatnich klas Państwa szkoły, którym w Systemie Informacji Oświatowej (SIO) zakończono przypisanie do oddziału wybierając status „Ukończył szkołę”. Ze względu na zapewnienie kompletności danych w kolejnych raportach, ważne jest nadawanie przez szkoły takiego statusu absolwentom do końca roku kalendarzowego, w którym zakończyli naukę.",
    ...
    if (szk$typ_szk[[1]] != "Branżowa szkoła I stopnia") {
      paste0(
        "\n\n* Ośrodek Przetwarzania Informacji przekazał dane dotyczące **",
        szk$l_abs_zrodla[[1]]$n_polon,
        szk$odmiany[[1]]$abs_polon,
...
cat("\n\n## Sposób prezentowania wyników\n\n",
    "Raport dotyczy absolwentów, którzy ukończyli szkołę w roku szkolnym ",
    params$rok - 1, "/", params$rok,
    ". W dalszej części raportu nazywamy ich „absolwentami z rocznika ",
    params$rok - 1, "/", params$rok,
    "”. Okres monitoringu, jakim zostali oni objęci, to półrocze po zakończeniu nauki, czyli okres lipiec - grudzień ",
    params$rok,
    ...
    "Przypisanie absolwentowi określonego statusu w danym miesiącu oznacza, że przynajmniej przez jeden dzień w tym miesiącu miał taki status. W przypadku łączenia w tym samym miesiącu nauki lub pracy z bezrobociem absolwentowi przypisywany jest status „nauka”, „praca” lub „nauka i praca”.\n\n",
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  cat("\n\n## Prośba o wypełnienie krótkiej ankiety zwrotnej dla IBE\n\n",
      ...
      "Ankieta dotyczy Państwa opinii i uwag odnośnie do raportu (dlatego przed wypełnieniem ankiety prosimy się z nim zapoznać), potrzeb dotyczących monitorowania losów absolwentów oraz   wykorzystania wyników.\n\n",
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  cat(
    ...
    "Wypełnienie ankiety zajmie około 10 minut. Ankieta będzie dostępna do końca sierpnia ",
    params$rok + 2,
    " roku.",
...
statusy_df = data.frame(
  ...
  `1. Tylko nauka` = "Status ten przypisywany jest osobom, o których wiadomo, że odbywają edukację formalną w Polsce i jednocześnie nie pracują.
  \nStatusu tego nie można przypisać osobom, które uczą się za granicą, ze względu na brak takiej informacji w danych rejestrowych, na których opiera się monitoring.
  \nW grupie tej mogą znaleźć się osoby pracujące, ale nie odprowadzają składek do ZUS, bowiem na podstawie tej informacji identyfikowanie są w monitoringu karier absolwentów osoby pracujące. Są to absolwenci zatrudnieni na umowy cywilnoprawne (wyjątkiem od opisanej sytuacji są osoby uczące się powyżej 26 roku życia, które przestają być zwolnienie z obowiązku opłacania składek do ZUS, a więc w monitoringu karier zaklasyfikowane są jako osoby pracujące, ale sytuacja ta dotyczyć może jedynie absolwentów liceów dla dorosłych oraz szkół policealnych), prowadzący działalność nierejestrowaną, ubezpieczeni z tytułu wykonywania pracy w innym systemie niż ZUS (np. KRUS), a także pracujący „na czarno”.
  \nPrzebywanie na urlopie macierzyńskim, rodzicielskim lub zwolnieniu lekarskim osoby będącej uczniem/studentem nie odbiera jej statusu osoby uczącej się.
  \nNa potrzeby wskaźnika, status „tylko nauka” przypisany jest także osobom zarejestrowanym jako bezrobotne, które jednocześnie kontynuują naukę w trybie niestacjonarnym.",
  `2. Nauka i praca` = "Status ten przypisywany jest osobom, które łączą naukę w ramach edukacji formalnej w Polsce i pracują w kraju. Statusu tego dotyczą zastrzeżenia opisane dla statusów „Tylko nauka” i „Tylko praca”.",
  `3. Tylko praca` = "Status ten przypisywany jest osobom, które nie odbywają edukacji formalnej w Polsce, ale pracują w kraju.
  \nPrzydzielanie tego statusu opiera się na informacji o odprowadzaniu składek do ZUS, co oznacza, że możliwe jest zidentyfikowanie osób pracujących na umowę o pracę, osób prowadzących działalność gospodarczą, a w przypadku osób powyżej 26 roku życia także absolwentów pracujących na umowę zlecenie. Nie jest możliwe zidentyfikowanie osób pracujących na umowę o dzieło, osób pracujących na umowę zlecenie poniżej 26 roku życia, absolwentów prowadzących działalność nierejestrowaną, pracujących na czarno, a także pracujących, ale ubezpieczonych w innym systemie niż ZUS (np. KRUS).
  \nMożliwa jest również sytuacja, w której osoby znajdujące się w tej grupie uczą się zagranicą.",
  `4. Bezrobocie` = "Status ten przypisywany jest osobom, które figurują w rejestrze ZUS z tytułu zarejestrowania w powiatowym urzędzie pracy jako bezrobotne - z wyjątkiem osób, które jednocześnie odbywają naukę formalną w Polsce. Absolwentom łączącym status osoby bezrobotnej z odbywaniem edukacji (co jest możliwe w przypadku nauki w szkole dla dorosłych, branżowej szkole II stopnia lub szkole policealnej, a także studiowania na studiach niestacjonarnych) przypisywany jest status „Tylko nauka”.
  \nNie jest wykluczone, że osoby zarejestrowane jako bezrobotne wykonują pracę „na czarno”.",
  `5. Brak danych o aktywności` = "Status ten przypisywany jest osobom, dla których brak informacji o odbywaniu nauki w Polsce lub wykonywaniu pracy związanej z odprowadzeniem składek do ZUS.
  \nNie jest wykluczone, że osoby te uczą się zagranicą, zaś jeśli się nie uczą, to pracują za granicą u zagranicznego pracodawcy, pracują na umowę o dzieło lub korzystając z ubezpieczenia w innym systemie niż ZUS (np. KRUS), wykonują pracę nierejestrowaną lub pracę „na czarno”.",
  check.names = FALSE) %>% 
