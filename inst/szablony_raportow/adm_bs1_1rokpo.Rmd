...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "%.\n\n",
    "Edukację w kolejnej szkole ponadpodstawowej z zatrudnieniem na podstawie umowy o pracę lub z wykonywaniem własnej działalności gospodarczej (samozatrudnieniem) łączyło ",
    ...
    "% absolwentów Państwa szkoły pracowało^[Wskaźnik osób pracujących i uczących się obejmuje jedynie osoby pracujące na podstawie umowy o pracę lub w ramach samozatrudnienia, ponieważ w przypadku umowy zlecenia osoba ucząca się zwolniona jest z opłacania składek. Z kolei wskaźnik osób pracujących, ale nie uczących się obejmuje także osoby pracujące na umowy zlecenia, ponieważ osoby te zobligowane są do samodzielnego odprowadzania składek do ZUS.], ale nie uczęszczało do kolejnej szkoły. Do grupy pracujących, ale nie kontynuujących edukacji formalnej należało ",
...
if (szk$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  ...
  tab <- tab_wykres(szk$E2_nauka_kontyn[[1]],
                   ...
                   80) %>%
    select(-typ) %>% 
    filter(!(name %in% c("Szkoła policealna", "Studia"))) %>% 
    arrange(desc(value))
...
```{r}
if (round(szk$E2_nauka_kontyn[[1]]$`Branżowa szkoła II stopnia` * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci & szk$liczebnosc_branze_kont[[1]]$n >= progLiczebnosci) {
...
```{r}
if (round(szk$E2_nauka_kontyn[[1]]$`Branżowa szkoła II stopnia` * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci & szk$branze_zawody[[1]]$n >= progLiczebnosci) {
  l_col <- length(szk$branze_zawody[[1]])
  if (length(szk$branze_zawody[[1]]) > 1 & l_col > 0) {
    ...
    for (i in 3:l_col) {
      naz_wiersze_nag <- paste(
        c(naz_wiersze_nag, paste(i - 2, "-", names(szk$branze_zawody[[1]])[i])),
        collapse = "\n")
    }
    wiersze_nag <- l_col - 2
  ...
  if (round(szk$E2_nauka_kontyn[[1]]$`Branżowa szkoła II stopnia` * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci & !(length(szk$branze_zawody[[1]]) > 1 & l_col > 0)) {
...
if (szk$Z9_mlod_ucz[[1]]$n + szk$Z9_mlod_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% uczyło się w niej odbywając praktyczną naukę zawodu na podstawie umowy o pracę w celu przygotowania zawodowego, zawartej z pracodawcą i było zatrudnionych we wrześniu ",
    params$rok,
    " roku.",
    sep = ""
  )
