...
params:
  plikZObiektami: ../../data/wskaznikiAdm5.RData
  obiektWskazniki: szk
...
`r ifelse(typDokumentu %in% "pdf", "\\vfill\n\\includegraphics[width=\\textwidth, keepaspectratio]{Belka-Losy-absolwentow-Kolor-PL}", "")`
...
## Status edukacyjno-zawodowy absolwentów ogółem
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "%.\n\n",
    "Naukę w systemie edukacji formalnej z zatrudnieniem na podstawie umowy o pracę lub z wykonywaniem własnej działalności gospodarczej (samozatrudnieniem) łączyło ",
    ...
    "% absolwentów Państwa branżowej szkoły II stopnia pracowało^[Wskaźnik osób pracujących i uczących się obejmuje jedynie osoby pracujące na podstawie umowy o pracę lub w ramach samozatrudnienia, ponieważ w przypadku umowy zlecenia osoba ucząca się zwolniona jest z opłacania składek. Z kolei wskaźnik osób pracujących, ale nie uczących się obejmuje także osoby pracujące na umowy zlecenia, ponieważ osoby te zobligowane są do samodzielnego odprowadzania składek do ZUS.], ale nie kontynuowało nauki. Do grupy pracujących, ale nie kontynuujących edukacji formalnej należało ",
    ...
    round(szk$S3_12[[1]]$brak_danych * 100),
    "% absolwentów Państwa szkoły oraz ",
    round(god$S3_12[[1]]$brak_danych * 100),
    "% absolwentów pozostałych branżowych szkół II stopnia z ",
    powiaty_dop,
    ".\n",
    sep = ""
    ...
    mutate(across(name,
                  ~ordered(name,
                           levels = c("Brak danych o aktywności",
                                      ...
                                      "Nauka i praca",
                                      "Tylko nauka"),
                           labels = c("brak_danych", "bezrob", "tylko_prac", "ucz_prac", "tylko_ucz")))) %>% 
    select(-lab) %>% 
    pivot_wider(names_from = name) %>% 
    select(typ, tylko_ucz, ucz_prac, tylko_prac, bezrob, brak_danych) %>% 
    mutate(across(tylko_ucz:brak_danych,
                  ~ round(. * 100))) %>%
    mutate(across(tylko_ucz:brak_danych,
                  ~ paste0(., "\\%"))) %>%
...
## Status edukacyjno-zawodowy absolwentów według zawodu kształcenia
    ...
    for (i in tab_s3_zaw_check$typ) {
      ...
      if (nrow(tab_god) > 0) {
        ...
        tabela <- rbind(tab_szk, tab_god) %>% 
          pivot_longer(cols = tylko_ucz:brak_danych, names_to = "name") %>% 
          mutate(across(name,
                        ~case_when(
                          ...
                          . %in% "brak_danych" ~ "Brak danych o aktywności"))) %>% 
...
## Status edukacyjno-zawodowy absolwentów miesiąc po miesiącu
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    ...
    if (length(unique(lip_sier)) %in% 1) {
      paste0(round(szk$S3_07[[1]]$ucz_prac * 100) + round(szk$S3_07[[1]]$tylko_prac * 100), "%.\n\n")
    } else {
      paste0(
        "od ",
        min(lip_sier),
...
if (min(szkoly_month$n) >= progLiczebnosci) {
  tab_szk <- szkoly_month %>%
    mutate(month = 7:12) %>% 
    select(-n) %>% 
    pivot_longer(tylko_ucz:brak_danych) %>% 
    mutate(across(name,
                  ~case_when(
                    . %in% "tylko_ucz" ~ "Tylko nauka",
                    . %in% "ucz_prac" ~ "Nauka i praca",
...
if (szk$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  kontyn <- as_tibble(szk$E2_nauka_kontyn[[1]]) %>% 
    select(-c(n, `Brak kontynuacji nauki`)) %>%  
    pivot_longer(everything()) %>% 
    mutate(nazwa = case_when(
      name %in% "Branżowa szkoła II stopnia" ~ paste0("* w branżowych szkołach II stopnia (", round(value[name == "Branżowa szkoła II stopnia"] * 100), "%)"),
      name %in% "Liceum dla dorosłych" ~ paste0("* w liceach dla dorosłych (", round(value[name == "Liceum dla dorosłych"] * 100), "%)"),
      name %in% "Szkoła policealna" ~ paste0("* w szkole policealnej (", round(value[name == "Szkoła policealna"] * 100), "%)"),
      name %in% "Studia" ~ paste0("* studiując na uczelni (", round(value[name == "Studia"] * 100), "%)"),
      name %in% "KKZ" ~ paste0("* na KKZ (", round(value[name == "KKZ"] * 100), "%)"),
      name %in% "KUZ" ~ paste0("* na KUZ (", round(value[name == "KUZ"] * 100), "%)"))
    )
  ...
  if (sposoby != 0) {
    ...
    if (round(szk$E2_nauka_kontyn[[1]]$`Brak kontynuacji nauki` * 100) == 50) {
      cat(
        "\n\nWśród absolwentów Państwa szkoły z rocznika ",
        rocznik,
...
if (szk$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  ...
  tab <- tab_wykres(szk$E2_nauka_kontyn[[1]],
                   ...
                   80) %>%
    select(-typ) %>% 
    filter(!(name %in% c("Branżowa szkoła II stopnia", "Liceum dla dorosłych"))) %>% 
    arrange(desc(value))
...
Kierunki studiów są przypisane do dziedzin i dyscyplin, zgodnie z klasyfikacją ministra właściwego ds. szkolnictwa wyższego i nauki (poszczególne dziedziny obejmują od kilku do kilkunastu dyscyplin).

```{r}
if (szk$liczebnosc_dziedziny[[1]]$n >= progLiczebnosci) {
  if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
    if (sum(szk$liczebnosc_dziedziny[[1]]$pct > 0) %in% 1) {
      cat(
        "Jedyną dziedziną, w której w grudniu ",
        params$rok,
        " roku studiowali absolwenci Państwa szkoły, była dziedzina ",
        szk$liczebnosc_dziedziny[[1]]$kont[szk$liczebnosc_dziedziny[[1]]$pct > 0],
        sep = ""
      )
    } else if (sum(szk$liczebnosc_dziedziny[[1]]$pct > 0) > 1 & sum(szk$liczebnosc_dziedziny[[1]]$pct %in% max(szk$liczebnosc_dziedziny[[1]]$pct)) %in% 1) {
      cat(
        "Dziedziną, w której w grudniu ",
        params$rok,
        " roku studiowało najwięcej absolwentów Państwa szkoły, była dziedzina ",
        szk$liczebnosc_dziedziny[[1]]$kont[szk$liczebnosc_dziedziny[[1]]$pct %in% max(szk$liczebnosc_dziedziny[[1]]$pct)],
        ". Kształciło się w niej ",
        round(max(szk$liczebnosc_dziedziny[[1]]$pct) * 100),
        "% spośród absolwentów, którzy w grudniu ",
        params$rok,
        " roku studiowali.",
        sep = ""
      )
    } else if (sum(szk$liczebnosc_dziedziny[[1]]$pct > 0) > 1 & sum(szk$liczebnosc_dziedziny[[1]]$pct %in% max(szk$liczebnosc_dziedziny[[1]]$pct)) > 1) {
      cat(
...
if (szk$liczebnosc_dziedziny[[1]]$n >= progLiczebnosci) {
  if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
    cat("Tabela ",
        ...
        params$rok,
        " roku - ogółem (%) - maksymalnie 10 dziedzin",
        sep = "")
...
```{r}
if (szk$liczebnosc_dyscypliny[[1]]$n >= progLiczebnosci) {
  if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
    if (sum(szk$liczebnosc_dyscypliny[[1]]$pct > 0) == 1) {
      cat(
        "Jedyną dyscypliną, w której w grudniu ",
        params$rok,
        " roku studiowali absolwenci Państwa szkoły, była dyscyplina ",
        szk$liczebnosc_dyscypliny[[1]]$kont[szk$liczebnosc_dyscypliny[[1]]$pct > 0],
        sep = ""
      )
    } else if (sum(szk$liczebnosc_dyscypliny[[1]]$pct > 0) > 1 & sum(szk$liczebnosc_dyscypliny[[1]]$pct %in% max(szk$liczebnosc_dyscypliny[[1]]$pct)) %in% 1) {
      cat(
        "Dyscypliną, w której w grudniu ",
        params$rok,
        " roku studiowało najwięcej absolwentów Państwa szkoły, była dyscyplina ",
        szk$liczebnosc_dyscypliny[[1]]$kont[szk$liczebnosc_dyscypliny[[1]]$pct %in% max(szk$liczebnosc_dyscypliny[[1]]$pct)],
        ". Kształciło się w niej ",
        round(max(szk$liczebnosc_dyscypliny[[1]]$pct) * 100),
        "% spośród absolwentów, którzy w grudniu ",
        params$rok,
        " roku studiowali.",
        sep = ""
      )
    } else if (sum(szk$liczebnosc_dyscypliny[[1]]$pct > 0) > 1 & sum(szk$liczebnosc_dyscypliny[[1]]$pct %in% max(szk$liczebnosc_dyscypliny[[1]]$pct)) > 1) {
      cat(
...
```{r}
if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci & szk$dyscypliny_kob[[1]]$n >= progLiczebnosci) {
  if (any(!is.na(szk$dyscypliny_kob[[1]]$pct))) {
    cat(
      "Poniższa tabela prezentuje rozkład dyscyplin, w których absolwentki Państwa szkoły najczęściej kontynuowały naukę.\n\nTabela ",
      tab_num(),
      ": Dyscypliny, w których absolwentki Państwa szkoły najczęściej studiowały w grudniu ",
      params$rok,
      " roku - kobiety (%) - maksymalnie 10 dyscyplin",
      sep = "")
...
```{r}
if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci & szk$dyscypliny_mez[[1]]$n >= progLiczebnosci) {
  if (any(!is.na(szk$dyscypliny_mez[[1]]$pct))) {
    cat(
      "Poniższa tabela prezentuje rozkład dyscyplin, w których absolwenci Państwa szkoły najczęściej kontynuowali naukę.\n\nTabela ",
      tab_num(),
      ": Dyscypliny, w których absolwenci Państwa szkoły najczęściej studiowali w grudniu ",
      params$rok,
      " roku - mężczyźni (%) - maksymalnie 10 dyscyplin",
      sep = "")
...
```{r}
if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci & szk$dyscypliny_zawody[[1]]$n >= progLiczebnosci) {
  l_col <- length(szk$dyscypliny_zawody[[1]])
  if (length(szk$dyscypliny_zawody[[1]]) > 1 & l_col > 0) {
    ...
    for (i in 3:l_col) {
      naz_wiersze_nag <- paste(
        c(naz_wiersze_nag, paste(i - 2, "-", names(szk$dyscypliny_zawody[[1]])[i])),
        collapse = "\n")
    }
    wiersze_nag <- l_col - 2
    names(wiersze_nag) <- paste0("% wśród studiujących absolwentów Państwa\nszkoły, którzy kształcili się w niej w zawodzie:\n", naz_wiersze_nag)
    
    nagl <- rep(1, l_col - 2, by = 1)
    names(nagl) <- lapply(1:(l_col - 2), as.character)
    ...
    tab <- szk$dyscypliny_zawody[[1]][-1] %>% 
      as_tibble() %>% 
      mutate(across(2:ncol(.),
                    ~paste0(round(. * 100), "\\%")))
    
    
    tab$kont <- unname(sapply(tab$kont, wrapper, width = 50))
    tab$kont <- linebreak(tab$kont, align = "l")
...
W przypadku absolwentów, którzy w danym miesiącu byli studentami lub uczniami szkoły policealnej, bierzemy pod uwagę jedynie pracę na etat oraz samozatrudnienie, ponieważ w ich przypadku brak informacji na temat zatrudnienia na podstawie umów cywilnoprawnych^[Ze względu na brak w rejestrze ZUS danych dotyczące ewentualnego wykonywania pracy na podstawie umów zlecenia przez uczniów i studentów do 26. roku życia.].
...
W tym rozdziale opisując sytuację zawodową absolwentów rozróżniono 4 formy wykonywania pracy:

* „Zatrudnieni tylko na UOP” - osoby zatrudnione tylko na umowę o pracę (UOP)
* „Tylko samozatrudnieni” - prowadzący własną działalność gospodarczą jako jedyną aktywność zawodową
* „Tylko inna forma” - osoby zatrudnione wyłącznie na podstawie pozostałych form umów - np. umowa zlecenie, umowa agencyjna, umowa o świadczenie usług i inne (np. osoby zatrudnione na umowie o staż z urzędu pracy, będące w czynnej służbie wojskowej czy otrzymujący stypendia sportowe)
* „Więcej niż jedna forma” - absolwenci łączący więcej niż jedną z powyższych form zatrudnienia

### Absolwenci pracujący i zarazem kontynuujący naukę
...
### Absolwenci pracujący ale nie kontynuujący nauki
...
if (szk$Z8_formy_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    ...
    case_when(
      szk_minus_god$nieucz_uop >= 3 & round(szk$Z8_formy_nie_ucz[[1]]$nieucz_uop * 100) == 0 ~ "częściej niż",
      szk_minus_god$nieucz_uop > 3 ~ "częściej niż",
      szk_minus_god$nieucz_uop <= -3 & round(szk$Z8_formy_nie_ucz[[1]]$nieucz_uop * 100) == 0 ~ "rzadziej niż",
    ...
    case_when(
      szk_minus_god$nieucz_samoz >= 3 & round(szk$Z8_formy_nie_ucz[[1]]$nieucz_samoz * 100) == 0 ~ "częściej niż",
      szk_minus_god$nieucz_samoz > 3 ~ "częściej niż",
      szk_minus_god$nieucz_samoz <= -3 & round(szk$Z8_formy_nie_ucz[[1]]$nieucz_samoz * 100) == 0 ~ "rzadziej niż",
...
```{r, out.width='100%'}
knitr::include_graphics("../boxplot_wzor.png")
```

### Absolwenci zatrudnieni na umowę o pracę i zarazem kontynuujący naukę
...
### Absolwenci zatrudnieni na umowę o pracę ale nie kontynuujący nauki
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  cat(
    ...
    round(szk$B2_bezrob[[1]]$pct[szk$B2_bezrob[[1]]$bezrobocie_r0_wrzgru == "Brak bezrobocia"] * 100),
    "% absolwentów w rozpatrywanym okresie w ogóle nie było zarejestrowanych jako osoby bezrobotne, natomiast ",
    round(szk$B2_bezrob[[1]]$pct[szk$B2_bezrob[[1]]$bezrobocie_r0_wrzgru == "4 miesiące"] * 100),
    "% pozostawało w rejestrze osób bezrobotnych przez cały ten okres.\n\nWśród absolwentów pozostałych branżowych szkół II stopnia z ",
    powiaty_dop,
    " analogiczne wartości wynosiły ",
    round(god$B2_bezrob[[1]]$pct[god$B2_bezrob[[1]]$bezrobocie_r0_wrzgru == "Brak bezrobocia"] * 100),
    "% i ",
    round(god$B2_bezrob[[1]]$pct[god$B2_bezrob[[1]]$bezrobocie_r0_wrzgru == "4 miesiące"] * 100),
    "%.\n\n",
    sep = ""
  )
} else {
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  wykres <- wyk_facet(tab_wyk,
    ...
    geom_text(data = tab_wyk,
              aes(x = mies, y = pct, label = lab, group = typ),
              nudge_y = 0.03,
              size = 2.5)
  ...
  tab_wyk <- tab_wyk %>%
    select(-lab) %>% 
    pivot_wider(names_from = mies, values_from = pct) %>%
    mutate(across(2:6,
                  ~ round(. * 100))) %>%
    mutate(across(2:6,
                  ~ paste0(., "\\%"))) %>%
...
woj <- woj %>% 
  filter(typ_szk %in% szk$typ_szk[[1]],
         teryt_woj_szk %in% szk$teryt_woj_szk[[1]])
```
...
if (woj$l_abs[[1]] < progLiczebnosci | woj$S3_12[[1]]$n < progLiczebnosci) {
  god2$S3_12[[1]] %>%
    as_tibble() %>% 
    select(n, tylko_ucz, ucz_prac, tylko_prac, bezrob, brak_danych) %>% 
...
tabela <- tabela %>% 
  ...
  arrange(desc(n)) %>% 
  select(nazwa_zaw, tylko_ucz, ucz_prac, tylko_prac, bezrob, brak_danych)
...
if (woj$l_abs[[1]] < progLiczebnosci | woj$E2_nauka_kontyn[[1]]$n < progLiczebnosci) {
  ...
  tabelka_bez_brak["Brak kontynuacji nauki"] <- NULL

  tabelka_bez_brak <- tabelka_bez_brak %>% 
    as_tibble() %>% 
    pivot_longer(everything()) %>% 
    arrange(desc(value))
  ...
  tabelka <- rbind(
    ...
    god2$E2_nauka_kontyn[[1]][-1] %>% 
      as_tibble() %>% 
      pivot_longer(everything()) %>% 
      filter(name %in% "Brak kontynuacji nauki"))
  ...
  tabelka <- tabelka %>% 
    filter(!(name %in% c("Liceum dla dorosłych", "Branżowa szkoła II stopnia"))) %>% 
    mutate(across(value,
                  ~paste0(round(. * 100), "\\%"))) %>%
    kable(format = "latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
          linesep = "",
  ...
  cat("\n\n\\textcolor{black}{\\relscale{0.75}Odsetki mogą sumować się do więcej niż 100\\% ze względu na możliwość kontynuowania edukacji w więcej niż jednej formie.}\n\n")
...
Wyróżniono 4 formy wykonywania pracy:
...
if (woj$l_abs[[1]] < progLiczebnosci | sum(woj$B2_bezrob[[1]]$n) < progLiczebnosci) {
  ...
  # god2$B2_bezrob[[1]]$bezrobocie_r0_wrzgru[1] <- "0 (brak bezrobocia)"
  
  tabelka <- god2$B2_bezrob[[1]] %>% 
    as_tibble() %>% 
    select(bezrobocie_r0_wrzgru, pct) %>% 
    mutate(across(pct,
                  ~paste0(round(. * 100), "\\%"))) %>% 
    kable("latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
          linesep = "",
  ...
  # woj$B2_bezrob[[1]]$bezrobocie_r0_wrzgru[1] <- "0 (brak bezrobocia)"
