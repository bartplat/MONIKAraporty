...
if (szk$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  nazwy_wierszy <- c(
    "Branżowa szkoła II stopnia", # tej etykiety nie będzie bo nie biorę tej kolumny
    "Liceum dla dorosłych", # tej etykiety nie będzie bo nie biorę tej kolumny
    "Szkoła policealna",
    "Szkoła wyższa",
...
if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
  ...
  if (length(szk$dyscypliny_zawody[[1]]) > 1 & l_col > 0) {
    ...
    tab <- szk$dyscypliny_zawody[[1]] %>% 
      as_tibble() %>% 
      mutate(across(2:ncol(.),
                    ~paste0(round(. * 100), "\\%")))
    
    
    tab$dyscyplina_wiodaca_kont <- unname(sapply(tab$dyscyplina_wiodaca_kont, wrapper, width = 50))
    tab$dyscyplina_wiodaca_kont <- linebreak(tab$dyscyplina_wiodaca_kont, align = "l")
