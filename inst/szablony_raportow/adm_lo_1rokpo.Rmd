...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% nie kontynuowało edukacji formalnej ani nie wykonywało pracy odnotowywanej przez ZUS^[Na podstawie umowy o pracę, na podstawie umowy cywilnoprawnej ani prowadząc własną działalność gospodarczą.]. W takiej samej sytuacji było ",
...
if (szk$liczebnosc_dziedziny[[1]]$n >= progLiczebnosci) {
  if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
    if (sum(szk$liczebnosc_dziedziny[[1]]$pct > 0) == 1) {
      cat(
...
```{r}
if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
  if (szk$dyscypliny_kob[[1]]$n >= progLiczebnosci) {
    cat(
...
```{r}
if (round(szk$E2_nauka_kontyn[[1]]$Studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
  if (szk$dyscypliny_mez[[1]]$n >= progLiczebnosci) {
    cat(
...
if (woj$l_abs[[1]] < progLiczebnosci | woj$S3_12[[1]]$n < progLiczebnosci) {
  god2$S3_12[[1]] %>%
    ...
    mutate(across(n, ~linebreak(paste0("Absolwenci LO\ndla młodzieży\nw całym kraju"), align = "l"))) %>%
    mutate(across(tylko_ucz:brak_danych,
                  ~paste0(round(. * 100), "\\%"))) %>%
    kable(format = "latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
          linesep = "",
...
if (woj$l_abs[[1]] < progLiczebnosci | woj$Z4_ucz[[1]]$n < progLiczebnosci) {
  ...
  tabelka <- tabelka %>% 
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>%
    add_header_above(c(" ", "Absolwenci LO z całego\nkraju kontynuujący naukę" = 1, "Absolwenci LO z całego\nkraju niekontynuujący nauki" = 1),
                     align = c("l", "c", "c"))
...
if (woj$l_abs[[1]] >= progLiczebnosci & woj$Z4_ucz[[1]]$n >= progLiczebnosci) {
  ...
  tabelka <- tabelka %>% 
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>%
    add_header_above(c(" ", "Absolwenci LO z województwa\nkontynuujący naukę" = 1, "Absolwenci LO z województwa\nniekontynuujący nauki" = 1),
                     align = c("l", "c", "c"))
